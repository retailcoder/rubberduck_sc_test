...
'works for any type, not just strings
...
' "Good Enough" version. A refactored, better performing object-oriented version exists
'...but uses way too many classes to be practical with VBA.
...
Public Function RequestUserInput(ByVal prompt As String, ByVal title As String, ByRef outResult As String, Optional ByVal default As String = vbNullString) As Boolean
    
    outResult = InputBox(prompt, title, default)
    
    Dim result As Boolean
    result = Not (StrPtr(outResult) = 0)
    
    RequestUserInput = result
    
End Function

Public Function RequestUserConfirmation(ByVal prompt As String, Optional ByVal title As String = "Confirmation required!") As Boolean
    RequestUserConfirmation = (MsgBox(prompt, vbExclamation + vbYesNo, title) = vbYes)
End Function
